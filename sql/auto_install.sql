...
CREATE TABLE `civicrm_inventory_batch` (
  ...
  `created_date` datetime DEFAULT CURRENT_TIMESTAMP COMMENT 'When was this item created',
...
CREATE TABLE `civicrm_inventory_billing_plans` (
  ...
  `frequency_unit` varchar(8) NOT NULL COMMENT 'Time units for recurrence of payment.',
  `frequency_interval` int unsigned NOT NULL COMMENT 'Number of time units for recurrence of payment.',
  `installments` int unsigned NULL COMMENT 'Total number of payments to be made. Set this to 0 if this is an open-ended commitment i.e. no set end date.',
  `intro_amount` decimal(20,2) NULL COMMENT 'The fair market value of this item.',
  `intro_frequency_unit` varchar(8) NULL COMMENT 'Time units for recurrence of payment.',
  `intro_frequency_interval` int unsigned NULL COMMENT 'Number of time units for recurrence of payment.',
  `intro_installments` int unsigned NULL COMMENT 'Total number of payments to be made. Set this to 0 if this is an open-ended commitment i.e. no set end date.',
...
CREATE TABLE `civicrm_inventory_shipment` (
  ...
  `created_date` timestamp DEFAULT CURRENT_TIMESTAMP COMMENT 'When was the shipment was created.',
...
CREATE TABLE `civicrm_inventory_product_membership` (
  ...
  `membership_type_id` int unsigned NOT NULL COMMENT 'Membership Type Associated with product.',
  `contact_id` int unsigned COMMENT 'FK to Contact',
  `is_product_serialize` tinyint NULL DEFAULT 0 COMMENT 'If electronic device, then set value to true.',
  `is_active` tinyint NULL DEFAULT 1 COMMENT 'If unchecked, product will be not be available with membership.',
  PRIMARY KEY (`id`),
